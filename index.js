...
weatherForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    resultDiv.innerHTML = weatherData
      .map((data) => {
        //Assings a different icon depending on the weather condition
        const weatherIcons = {
          Clear: "fas fa-sun text-warning", // Sun for clear weather
          Clouds: "fas fa-cloud text-secondary", // Cloud for cloudy weather
          Rain: "fas fa-cloud-showers-heavy text-primary", // Rain for rainy weather
          Snow: "fas fa-snowflake text-info", // Snowflake for snowy weather
          Thunderstorm: "fas fa-bolt text-danger", // Lightning for storms
          Drizzle: "fas fa-cloud-rain text-blue-300", // Light rain/drizzle
          Mist: "fas fa-smog text-gray-400", // Fog/Mist
        };

        // Get the correct icon class, or default to "fas fa-question-circle"
        const weatherMain = data.weather[0].main;
        const weatherIcon =
          weatherIcons[weatherMain] || "fas fa-question-circle";
    ...
    <div class="card result-card p-3">
        ...
        <p class="mb-2"><i class="${weatherIcon}"></i> ${data.weather[0].description}</p>
    ...
    const colors = temperatures.map((temp) => {
      ...
      if (temp > 20 && temp <= 25) return "#FFEB3B"; // Yellow
      if (temp > 25 && temp <= 30) return "#FFA500"; // Orange
